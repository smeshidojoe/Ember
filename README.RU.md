...
| Bluesky | видео (HLS), картинки, гифки | картинки отдаются исходником автора, а не пережатой CDN-версией |
...
- **Берите cookies с отдельного аккаунта, а не с основного.** Сервисы считают лимиты
  и банят по аккаунту, а автоматическое извлечение и выглядит как автоматика. Строже
  всех Instagram: после некоторого числа запросов он отвечает `feedback_required`
  («мы ограничиваем, как часто можно делать некоторые вещи»), и дальше у этого
  аккаунта отваливаются *все* пути — Ember скатывается до обложки поста и помечает
  её `is_preview=True`. Лимит привязан именно к аккаунту, поэтому второй профиль
  браузера с отдельным логином продолжает работать, пока первый отходит (обычно
  сутки). Аккаунт, который душат раз за разом, могут ограничить насовсем — не
  подставляйте рабочий.
